...
namespace BarcodeExport.Sources
{
	...
	public class Code39
	{
        ...
        public Bitmap GenerateFixedBarcode(string barCode, int width, int height)
        {

            int bcodeWidth = width;
            int bcodeHeight = height;
        ...
        private Bitmap CreateFixedImageContainer(string barCode, ref int bcodeWidth, ref int bcodeHeight)
        {
            ...
            {
                _titleSize = objGraphics.MeasureString(_titleString, _titleFont);
                width = (int)_titleSize.Width;
                height = (int)_titleSize.Height + _itemSepHeight;
            }

            _barCodeSize = objGraphics.MeasureString(barCode, Code39Font);
            width = Max(width, (int)_barCodeSize.Width);
            height += (int)_barCodeSize.Height;

            if (_showCodeString)
            {
                _codeStringSize = objGraphics.MeasureString(barCode, _codeStringFont);
                width = Max(width, (int)_codeStringSize.Width);
                height += (_itemSepHeight + (int)_codeStringSize.Height);
            }
